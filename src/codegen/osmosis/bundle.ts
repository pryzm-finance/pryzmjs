import * as _156 from "./epochs/v1beta1/genesis";
import * as _157 from "./epochs/v1beta1/query";
import * as _158 from "./tokenfactory/v1beta1/authorityMetadata";
import * as _159 from "./tokenfactory/v1beta1/genesis";
import * as _160 from "./tokenfactory/v1beta1/params";
import * as _161 from "./tokenfactory/v1beta1/query";
import * as _162 from "./tokenfactory/v1beta1/tx";
import * as _393 from "./tokenfactory/v1beta1/tx.amino";
import * as _394 from "./tokenfactory/v1beta1/tx.registry";
import * as _395 from "./epochs/v1beta1/query.lcd";
import * as _396 from "./tokenfactory/v1beta1/query.lcd";
import * as _397 from "./epochs/v1beta1/query.rpc.Query";
import * as _398 from "./tokenfactory/v1beta1/query.rpc.Query";
import * as _399 from "./tokenfactory/v1beta1/tx.rpc.msg";
import * as _455 from "./lcd";
import * as _456 from "./rpc.query";
import * as _457 from "./rpc.tx";
export namespace osmosis {
  export namespace epochs {
    export const v1beta1 = {
      ..._156,
      ..._157,
      ..._395,
      ..._397
    };
  }
  export namespace tokenfactory {
    export const v1beta1 = {
      ..._158,
      ..._159,
      ..._160,
      ..._161,
      ..._162,
      ..._393,
      ..._394,
      ..._396,
      ..._398,
      ..._399
    };
  }
  export const ClientFactory = {
    ..._455,
    ..._456,
    ..._457
  };
}