...
export const Module = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          break;
        case 2:
          message.moduleAccountPermissions.push(ModuleAccountPermission.decode(reader, reader.uint32(), useInterfaces));
          break;
        case 3:
  ...
  toAmino(message: Module, useInterfaces: boolean = true): ModuleAmino {
    ...
    if (message.moduleAccountPermissions) {
      obj.module_account_permissions = message.moduleAccountPermissions.map(e => e ? ModuleAccountPermission.toAmino(e, useInterfaces) : undefined);
    } else {
...
export const ModuleAccountPermission = {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number, useInterfaces: boolean = true): ModuleAccountPermission {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  toAmino(message: ModuleAccountPermission, useInterfaces: boolean = true): ModuleAccountPermissionAmino {
    const obj: any = {};
  ...
  toAminoMsg(message: ModuleAccountPermission, useInterfaces: boolean = true): ModuleAccountPermissionAminoMsg {
    return {
      type: "cosmos-sdk/ModuleAccountPermission",
      value: ModuleAccountPermission.toAmino(message, useInterfaces)
    };
  },
  fromProtoMsg(message: ModuleAccountPermissionProtoMsg, useInterfaces: boolean = true): ModuleAccountPermission {
    return ModuleAccountPermission.decode(message.value, undefined, useInterfaces);
  },
