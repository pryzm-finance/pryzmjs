...
import { MsgSubmitQueryResponse } from "./types/prismcore/icq/tx";
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgSubmitQueryResponse({ value }: msgSubmitQueryResponseParams): EncodeObject {
			try {
				return { typeUrl: "/prismfinance.prismcore.icq.MsgSubmitQueryResponse", value: MsgSubmitQueryResponse.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			PrismfinancePrismcoreIcq: new SDKModule(test)
		},
		registry: msgTypes
  }
}
