...
import {newPrismSigningClient} from "../src"
...
async function main() {
    ...
    const msg1 = await signingClient.CosmosBankV1Beta1.tx.msgSend({
        value: {
            amount: [{
                amount: "10",
                denom: "uprism"
    ...
    const msg2 = await signingClient.CosmosBankV1Beta1.tx.msgSend({
        value: {
            amount: [{
                amount: "20",
                denom: "uprism"
    ...
    const tx_result = await signingClient.signAndBroadcast([msg1, msg2], {
        amount: [{
            amount: "0",
            denom: "uprism"
        }],
